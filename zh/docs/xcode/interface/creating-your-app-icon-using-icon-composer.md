...
系统会从你应用包中的单一 Icon Composer 文件，为不同平台、外观和尺寸渲染你的应用图标。对于不支持相同图标和小组件样式外观及 Liquid Glass 材质的早期版本，Xcode 会在构建时根据 Icon Composer 文件生成应用图标图片。
如果你选择不使用 Icon Composer，你仍然可以在项目中使用包含单独应用图标图片的 AppIcon 资源目录，并让系统应用 Liquid Glass 材质。
如需了解更多信息，请参阅以下资源：

- 关于设计应用图标的指导，请参阅人机界面指南 > 基础知识 > 应用图标 。
- 关于将旧版应用图标转换为使用 Liquid Glass 材质，请参阅采用 Liquid Glass > 应用图标 。
- 关于 Liquid Glass 和 Icon Composer 的更多信息，请观看应用图标新外观介绍和使用 Icon Composer 创建图标 。
  对于仍然使用 AppIcon 资源目录的 tvOS 和 visionOS 目标，请参阅使用资源目录配置应用图标 。
...
- 从你在 Apple Design Resources 下载的应用图标模板开始，该模板包含最新的网格、形状和画布尺寸。
...
要在最新版本的 Xcode 中启动 Icon Composer，选择 Xcode > 打开开发者工具 > Icon Composer。如果你没有安装 Xcode，可以从下载 > 应用程序下载 Icon Composer。
...
你可以继续使用 Icon Composer 对你的应用图标进行微调，并稍后将其添加到 Xcode 项目中。要将你的应用图标添加到 Xcode 项目并与应用目标关联，请参阅将你的 Icon Composer 文件添加到 Xcode 项目 。
...
A screenshot that shows the group and layer hierarchy after you drop a folder on the sidebar. There are three groups with different numbers of layers. The groups and layers appear alphabetically from bottom to top in the sidebar.
...
A screenshot of the Add button pop-up menu at the bottom of the sidebar with the Group menu item selected.
...
A screenshot of the sidebar with callouts that show the groups and layers in the Landmarks sample app icon.
您可以使用侧边栏进行以下编辑：
...
A screenshot that shows the Landmarks icon preview when you select the default appearance.
A screenshot that shows the Landmarks icon preview when you select the dark appearance.
A screenshot that shows the Landmarks icon preview when you select the mono appearance.
要预览清晰和着色变体，点击单色，然后点击选项。在弹出的对话框中，选择浅色或深色，切换着色开关，并使用滑块选择着色颜色。
...
A screenshot with callouts that shows the background, grid, lighting angle, and icon size controls.
你可以使用工具栏控件设置以下内容：
...
- 要放大或缩小，请从大小弹出菜单中选择一个百分比。
  ...
  A screenshot of the canvas that shows the mono appearance over a blue background image.
...
对于组，你可以自定义所有 Liquid Glass 材质选项。在侧边栏中选择一个组，然后在检查器的模式弹出菜单中选择“单独”或“组合”。“单独”会将效果分别应用于组中的每个图层。“组合”会将效果作为一个对象应用于组中的所有图层。
Individual
Combined
A screenshot of the preview that shows Liquid Glass applied to individual layers in a group.
A screenshot that shows the Mode set to Individual.
...
Effects on
Effects off
A screenshot of a preview with Liquid Glass effects on for all layers.
A screenshot of the Liquid Glass settings for a layer with Effects toggled on.
...
A screenshot that shows Dark selected from the Grid pop-up menu at the top of the canvas.
...
A screenshot that shows the Layout section under Composition with the x, y, and scale settings.
...
A screenshot that shows custom settings for dark and mono appearances when you choose All from the Color pop-up menu.
...
A screenshot that shows Dark selected from the Color pop-up menu when you select the dark appearance in the canvas.
...
有关在 Xcode 中运行应用的更多信息，请参阅在模拟器或设备上运行你的应用 。
